...
from userbot.utils.exceptions import userbotBotNotFound
...
class _Module:
    def __init__(self, cat: str, name: str):
        self.cat = cat
        self.name = name
        self._path = f"userbot.plugins.{cat}.{name}"
...
_userbot_STATUS = get_collection("userbot_STATUS")


async def _set_running(is_running: bool) -> None:
    await _userbot_STATUS.update_one(
        {'_id': 'userbot_STATUS'},
...
async def _is_running() -> bool:
    if config.ASSERT_SINGLE_INSTANCE:
        data = await _userbot_STATUS.find_one({'_id': 'userbot_STATUS'})
        if data:
...
async def _wait_for_instance() -> None:
    ...
    while await _is_running():
        _LOG.info("Waiting for the Termination of "
                  f"previous userbot instance ... [{timeout} sec]")
        time.sleep(timeout)
...
class _Abstractuserbot(Methods):
    ...
    async def _load_plugins(self) -> None:
        ...
        _MODULES.clear()
        base = os.path.join("userbot", "plugins")
    ...
    def __eq__(self, o: object) -> bool:
        return isinstance(o, _Abstractuserbot) and self.id == o.id

    def __hash__(self) -> int:  # pylint: disable=W0235
        return super().__hash__()


class userbotBot(_Abstractuserbot):
    """ userbotBot, the bot """

    def __init__(self, **kwargs) -> None:
        super().__init__(name="userbotBot", in_memory=True, **kwargs)

    @property
    def ubot(self) -> 'userbot':
        """ returns userbot """
        return self._bot


class userbot(_Abstractuserbot):
    """ userbot, the userbot """
    ...
    def __init__(self) -> None:
        ...
        if config.SESSION_STRING and config.BOT_TOKEN:
            ...
            kwargs['bot'] = userbotBot(bot=self, **kwargs)
    ...
    @property
    def bot(self) -> Union['userbotBot', 'userbot']:
        """ returns userbotbot """
        if self._bot is None:
            if config.BOT_TOKEN:
                return self
            raise userbotBotNotFound("Need BOT_TOKEN ENV!")
    ...
    def _get_log_client(self) -> _Abstractuserbot:
        return self.bot if self.has_bot else self

    async def _log_success(self) -> None:
        # pylint: disable=protected-access
        await self._get_log_client()._channel.log("<pre>userbot started successfully</pre>")
    ...
    def begin(self, coro: Optional[Awaitable[Any]] = None) -> None:
        ...
        def _handle(num, _) -> None:
            _LOG.info(
                f"Received Stop Signal [{signal.Signals(num).name}], Exiting userbot ...")
        ...
        with suppress(asyncio.exceptions.CancelledError, RuntimeError):
            if coro:
                ...
                _LOG.info(f"Idling userbot - {mode}")
