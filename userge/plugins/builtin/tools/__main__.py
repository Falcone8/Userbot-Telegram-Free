...
from userbot import userbot, Message, logging, config, pool, get_collection
...
@userbot.on_cmd("vesto", about={'header': "View edit sleep timeout"})
...
async def search(message: Message):
    ...
    data = list(userbot.manager.loaded_plugins if plugins else userbot.manager.loaded_commands)
...
@userbot.on_cmd("logs", about={
    'header': "check userbot logs",
    'flags': {
...
async def check_logs(message: Message):
    ...
    if '-h' in message.flags and config.HEROKU_APP:
        ...
        await message.client.send_as_file(chat_id=message.chat.id,
                                          text=logs,
                                          filename='userbot-heroku.log',
                                          caption=f'userbot-heroku.log [ {limit} lines ]')
    else:
        filename = f"{'loader' if '-l' in message.flags else 'userbot'}.log"
