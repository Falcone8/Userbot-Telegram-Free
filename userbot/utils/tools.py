...
def get_file_id_of_media(message: 'userbot.Message') -> Optional[str]:
    """ get file_id """
...
async def take_screen_shot(video_file: str, duration: int, path: str = '') -> Optional[str]:
    ...
    thumb_image_path = path or join(
        userbot.config.Dynamic.DOWN_PATH,
...
def is_command(cmd: str) -> bool:
    commands = userbot.userbot.manager.loaded_commands
    key = userbot.config.CMD_TRIGGER + cmd
    _key = userbot.config.SUDO_TRIGGER + cmd
