...
@userbot.on_cmd("help", about={
    'header': "Guide to use userbot commands",
...
async def helpme(message: Message) -> None:  # pylint: disable=missing-function-docstring
    plugins = userbot.manager.loaded_plugins

    if userbot.has_bot and '-i' in message.flags:
        bot = (await userbot.bot.get_me()).username
        menu = await userbot.get_inline_bot_results(bot)
    ...
    if not message.input_str:
        ...
        cat_plugins = userbot.manager.get_plugins()
...
({size}) <b>Command{'s' if size > 1 else ''}</b>\n\n"""
            ...
            for name, cmd in userbot.manager.loaded_commands.items():
                for _ in triggers:
                    name = name.lstrip(_)
...
if userbot.has_bot:
    ...
    def check_owner(func):
        async def wrapper(_, c_q: CallbackQuery):
            if c_q.from_user and c_q.from_user.id in config.OWNER_ID:
                ...
                user_dict = await userbot.bot.get_user_dict(config.OWNER_ID[0])
                await c_q.answer(
                    f"Only {user_dict['flname']} Can Access this...! Build Your Own @Theuserbot 🤘",
                    show_alert=True)

        return wrapper

    @userbot.bot.on_message(filters.private & filters.user(
    ...
    async def pm_help_handler(_, msg: Message):
        ...
        commands = userbot.manager.loaded_commands
    ...
    async def callback_next_prev(callback_query: CallbackQuery):
        ...
        if len(pos_list) == 1:
            buttons = parse_buttons(p_num, cur_pos,
                                    ...
                                    userbot.manager.get_all_plugins())
        elif len(pos_list) == 2:
            buttons = parse_buttons(p_num, cur_pos,
                                    lambda x: f"🗃 {x}",
                                    userbot.manager.get_all_plugins()[pos_list[-1]])
        elif len(pos_list) == 3:
    ...
    async def callback_manage(callback_query: CallbackQuery):
        ...
        if len(pos_list) == 4:
            if is_filter(pos_list[-1]):
                flt = userbot.manager.filters[pos_list[-1]]
            else:
                flt = userbot.manager.commands[pos_list[-1]]

            getattr(flt, task)()
            text, buttons = filter_data(cur_pos)

        else:
            plg = userbot.manager.plugins[pos_list[-1]]
    ...
    @userbot.bot.on_callback_query(filters=filters.regex(pattern=r"^mm$"))
    @check_owner
    async def callback_mm(callback_query: CallbackQuery):
        await callback_query.edit_message_text(
            "🖥 **userbot Main Menu** 🖥", reply_markup=InlineKeyboardMarkup(main_menu_buttons()))

    @userbot.bot.on_callback_query(filters=filters.regex(pattern=r"^chgclnt$"))
    @check_owner
    async def callback_chgclnt(callback_query: CallbackQuery):
        if not userbot.dual_mode:
    ...
    def category_data(cur_pos: str):
        pos_list = cur_pos.split('|')
        plugins = userbot.manager.get_all_plugins()[pos_list[1]]
    ...
    def plugin_data(cur_pos: str, p_num: int = 0):
        ...
        pos_list = cur_pos.split('|')
        plg = userbot.manager.plugins[pos_list[2]]
...
"""
    ...
    @userbot.bot.on_inline_query(group=1)
        ...
        results = [
            InlineQueryResultArticle(
                ...
                input_message_content=InputTextMessageContent(
                    "**Here's how to setup userbot** 😎"
                ),
                url="https://github.com/userbotTeam/userbot",
        ...
        if inline_query.from_user and inline_query.from_user.id in config.OWNER_ID:
            ...
            if "msg.err" in inline_query.query:
                ...
                if cmd and err_text and is_command(cmd):
                    bot_username = (await userbot.bot.get_me()).username
