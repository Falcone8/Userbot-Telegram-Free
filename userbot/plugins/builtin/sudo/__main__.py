...
@userbot.on_cmd("sudo", about={'header': "enable / disable sudo access"}, allow_channels=False)
...
@userbot.on_cmd("vsudo", about={'header': "view sudo users"}, allow_channels=False)
...
async def add_sudo_cmd(message: Message):
    ...
    if '-all' in message.flags:
        ...
        for c_d in list(userbot.manager.loaded_commands):
...
@userbot.on_cmd("vscmd", about={'header': "view sudo cmds"}, allow_channels=False)
