...
from userbot.utils import extract_entities
...
@userbot.on_cmd("shutdown", about={'header': "shutdown userbot :)"}, allow_channels=False)
...
async def die_(message: Message) -> None:
    """ set offline timeout to die userbot """
    global MAX_IDLE_TIME  # pylint: disable=global-statement
...
@userbot.on_cmd("listdisabled", about={'header': "List all disabled chats."})
...
@userbot.on_cmd("convert_usermode", about={
...
@userbot.on_cmd("convert_botmode", about={
...
async def convert_botmode(msg: Message):
    ...
    try:
        async with userbot.conversation('botfather') as conv:
            try:
                await conv.send_message('/start')
            except YouBlockedUser:
                await userbot.unblock_user('botfather')
                await conv.send_message('/start')
            ...
            if 'Sorry' in response.text:
                ...
                await userbot.promote_chat_member(config.LOG_CHANNEL_ID, username)
