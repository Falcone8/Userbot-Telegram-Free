...
from userbot.plugins.builtin import sudo, system
...
async def _raise_func(r_c: Union['_client.userbot', '_client.userbotBot'],
...
async def _bot_is_present(r_c: Union['_client.userbot', '_client.userbotBot'],
...
async def _both_are_admins(r_c: Union['_client.userbot', '_client.userbotBot'],
                           r_m: RawMessage, is_bot: bool) -> bool:
...
async def _both_have_perm(flt: Union['types.raw.Command', 'types.raw.Filter'],
                          r_c: Union['_client.userbot', '_client.userbotBot'],
                          r_m: RawMessage, is_bot: bool) -> bool:
    if not await _bot_is_present(r_c, r_m, is_bot):
        return False
    try:
...
class RawDecorator(RawClient):
    ...
    def on_exit(self, callback: Callable[[], Awaitable[Any]]) -> None:
        """ set a callback to calls when the userbot is exiting """
    ...
    def _build_decorator(self,
        ...
        def decorator(func: _PYROFUNC) -> _PYROFUNC:
            async def template(r_c: Union['_client.userbot', '_client.userbotBot'],
                               r_m: RawMessage) -> None:
                    ...
                    async with await _get_lock(str(flt)):
                        ...
                        if cond:
                            if config.Dynamic.USER_IS_PREFERRED:
                                if isinstance(r_c, _client.userbotBot):
                                    return
